...
// if you use for previous one, need to use .get() method, but for each loop no need, directly it gets itself.
		}
		...
		for(Character w : list1) {
// for each loop, need to use all elements			
			if(w=='a' || w=='e' || w=='i' || w=='o' || w=='u') {
				continue;// skip, dont run if the condition is true
			}
		...
		int sum = 0; 
		for(Character w : list1) {
			sum = sum + w;// w did not used (int) bcoz if used with math operation and char, then char will be executed as value 
		}
