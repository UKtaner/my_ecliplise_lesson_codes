	...
	/*
 	...
 	2)Polymorphism: Overloading + Overriding 
 		...
 		c)What is the difference between "Overloading" and "Overriding"?
 			...
 			iii)final methods can be overloaded only not allow to touch the both, but final methods cannot be overridden.
 			iv)Overloading happens in Compile Time, but Overriding happens in Run Time 
 			v)Overloading is faster than Overriding
 			vi) if there is no inheritance, there is no overriding.Using multi classes.
 			vii)overloading only works in one class so no need multi class, no relation from other classes.
